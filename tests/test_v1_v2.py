...
class AgentLoopTeachingSuiteTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_v1_basic_turn(self) -> None:
        loop = V1BasicLoop(client=FakeClientV1(), model_name="test-model")
        text = await loop.run_turn("hello")
        self.assertEqual(text, "echo:hello")
        self.assertEqual(len(loop.get_messages()), 2)
        self.assertEqual(loop.get_messages()[0]["role"], "user")
        self.assertEqual(loop.get_messages()[1]["role"], "assistant")

    async def test_v2_tool_roundtrip(self) -> None:
        loop = V2ToolsLoop(
            client=FakeClientV2(),
            model_name="test-model",
        )
        text = await loop.run_turn("what is 2+3")
