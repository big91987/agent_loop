...
from loops.agent_loop_v6_2 import V6_2
...
class RefreshUI:
    ...
    def render(self) -> None:
        ...
        sep = "-" * width
        header_lines = [
            f"agent-loop v6.2 | model={self.model_name}",
            f"log: {self.log_path}",
...
def _token_snapshot(loop: V6_2) -> dict[str, int | bool]:
...
def _restore_token_baseline(loop: V6_2) -> None:
...
def _restore_token_baseline_from_record(loop: V6_2, record: SessionRecord) -> None:
...
def _restore_short_memory_state_from_record(loop: V6_2, record: SessionRecord) -> None:
...
def _reset_token_baseline(loop: V6_2) -> None:
...
def _setup_readline(*, history_file: Path, loop: V6_2, store: SessionStoreV6) -> None:
    if readline is None:
        return
...
def _resolve_workspace_path(path: str, workspace_path: str | None) -> str:
    src = str(path).strip()
    if not src:
        return src
    p = Path(src)
    if p.is_absolute() or not workspace_path:
        return str(p)
    return str((Path(workspace_path) / p).resolve())


async def async_main() -> int:
    parser = argparse.ArgumentParser(description="v6.2 CLI (session + short-memory compaction)")
    ...
    parser.add_argument("--history-file", default="./logs/cli_v6_2_history.txt", help="Readline history file")
    parser.add_argument(
        "--rehydrate-history",
        type=int,
        default=100,
    ...
    effective_sessions_dir = _resolve_workspace_path(args.sessions_dir, workspace_path)
    effective_history_file = _resolve_workspace_path(args.history_file, workspace_path)
    effective_memory_store_path = _resolve_workspace_path(cfg.memory_store_path, workspace_path)

    logger, log_path = create_session_logger(log_dir=effective_log_dir, debug=args.debug)
    logger.info("startup loop=v6.2 model=%s provider=%s", cfg.model_name, cfg.provider)
    ...
    loop = V6_2(
        ...
        memory_system_dir=cfg.memory_system_dir,
        memory_store_path=effective_memory_store_path,
        workspace_path=workspace_path,
    )

    store = SessionStoreV6(effective_sessions_dir)
    if args.session:
        ...
        record = store.create(model_name=cfg.model_name, loop_version="v6.2", persist=False)
        loop.set_memory_session(record.session_id)
        _reset_token_baseline(loop)

    _setup_readline(history_file=Path(effective_history_file), loop=loop, store=store)
    ...
    if not ui.enabled:
        ui.add(f"agent-loop suite started | loop=v6.2 | model={cfg.model_name}")
    ...
    try:
        while True:
            ...
            if user_input.startswith("/memory "):
                ...
                if action == "status":
                    st = loop.get_short_memory_state()
                    lt = loop.get_memory_system_status()
                    ...
                    ui.add(
                        ...
                        f"long_records={lt.get('records_alive', 0)}",
                    )
                    ui.add(
                        "[MEMORY-LONG] "
                        f"system_dir={lt.get('system_dir')} | "
                        f"store={lt.get('path')} | "
                        f"session_id={lt.get('session_id')}",
                    )
